...
- [ ] **Test 1**: API 클라이언트 초기화 테스트
  - [ ] 환경변수에서 API 키를 읽어오는지 확인
  - [ ] API 기본 URL이 올바른지 확인

- [ ] **Test 2**: 이미지 검색 함수 테스트
  - [ ] 검색어를 받아 Pixabay API 호출
  - [ ] 응답 데이터 파싱 확인
  - [ ] API 에러 핸들링 (네트워크 에러, 401, 404 등)

- [ ] **Test 3**: 페이지네이션 파라미터 테스트
  - [ ] page와 per_page 파라미터가 올바르게 전달되는지 확인

#### 1.2 UI 렌더링 모듈 (Red → Green → Refactor)
- [ ] **Test 4**: 검색창 렌더링 테스트
  - [ ] 입력 필드와 검색 버튼이 렌더링되는지 확인

- [ ] **Test 5**: 이미지 그리드 렌더링 테스트
  - [ ] 이미지 배열을 받아 그리드로 렌더링
  - [ ] 각 이미지 카드에 필요한 정보(썸네일, 태그 등) 표시

- [ ] **Test 6**: 스켈레톤 로딩 UI 테스트
  - [ ] 로딩 상태일 때 스켈레톤 카드 표시
  - [ ] 로딩 완료 시 실제 데이터로 교체

- [ ] **Test 7**: 에러 메시지 표시 테스트
  - [ ] 에러 발생 시 사용자 친화적 메시지 표시

- [ ] **Test 8**: 페이지네이션 UI 테스트
  - [ ] 이전/다음 버튼 렌더링
  - [ ] 현재 페이지 번호 표시
  - [ ] 첫 페이지/마지막 페이지에서 버튼 비활성화

#### 1.3 통합 테스트
- [ ] **Test 9**: 검색 워크플로우 통합 테스트
  - [ ] 검색어 입력 → 로딩 표시 → 결과 렌더링
  - [ ] 검색 결과 없음 처리
